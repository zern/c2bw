...
- **双工作模式支持**：
  ...
  - **PDF 书签大纲智能继承与重映射 (v4.0 新增)**：从原 PDF 自动提取多层级目录书签树，重构新 PDF 时自动导入并设置 `/UseOutlines`；双页分割裁切时自动将书签对应映射至目标切分页（默认绑定 `_B` 页面），不裁切或单页时保持对应原页码。
- **丰富的图像格式兼容**：
...
- **高质量黑白二值化（新增 Wolf 局部自适应）**：
  - **OTSU 自适应算法（默认）**：全图全局自适应最佳阈值，适合常规印刷体与纸质均匀文献；
  - **Wolf 局部自适应算法 (v4.0 新增)**：基于滑动窗口局部均值与对比度动态计算阈值，针对古籍纸张发黄、严重水渍霉斑、光照不均与边缘暗角具有极佳的文字笔画还原与污渍剔除能力；
  - **5 组精选预设组合**：提供「干净 (w=41, k=0.22)」、「标准 (w=51, k=0.30)」、「淡墨 (w=61, k=0.38)」、「污渍抑制 (w=71, k=0.24)」、「阴影修复 (w=41, k=0.34)」，并支持手动微调窗口与系数；
  - **自定义全局固定阈值**：支持 0~100% 阈值手动微调；
  - 二值化结果统一输出为标准 1 位 Group 4 TIFF，极佳保留笔画细节。
...
- **色彩处理关闭时的文件大小优化体系**：
  ...
  - **精简尺寸（适合手机）**：在裁切前使用最优算法（Lanczos）缩减尺寸，双页跨页大图等比压缩为 2160px 宽（切分后单页约 1080px），开启分页处理后判定为单页原图的调整为 1080px 宽（宽度 ≤1080px 不放大，保持原比例），输出为质量 75 的渐进式（Progressive）JPEG，兼顾高清晰度与极佳的文件压缩率；
...
build.bat              一键批量构建默认版本（Nuitka 版 + Win7 版）
build_nuitka.bat        Windows 10/11 Nuitka C++ 原生独立版一键构建脚本
...
### 1. Nuitka C++ 原生独立版（Win10 / Win11 推荐）
